...
namespace MediCare.Pages.Doctor
{
    public partial class Dashboard : System.Web.UI.Page
    {
        ...
        private int GetDoctorId(SqlConnection conn)
        {
            ...
            using (var cmd = new SqlCommand("SELECT DoctorId FROM [dbo].[Doctors] WHERE UserId=@uid", conn))
            {
                ...
                try
                {
                    ...
                    Execute(conn,
                        @"UPDATE [dbo].[Appointments]
                          SET Status=@s
                          WHERE AppointmentId=@aid AND DoctorId=@did",
                        ("@s", newStatus),
                        ("@aid", appointmentId),
                        ("@did", doctorId));
                        ...
                        if (appDate != DateTime.MinValue)
                        {
                            Execute(conn,
                                @"DELETE FROM [dbo].[DoctorAvailability]
                                  WHERE DoctorId=@did AND StartTime=@s AND EndTime=@e",
                                ("@did", doctorId),
                                ("@s", appDate),
                                ("@e", appDate.AddHours(1)));
                        }
                    }
                    ...
                    using (var cmd = new SqlCommand(@"
                        ...
                        FROM [dbo].[Appointments] a
                        INNER JOIN [dbo].[Patients] p ON a.PatientId = p.PatientId
                        WHERE a.AppointmentId=@aid", conn))
                    {
                        cmd.Parameters.AddWithValue("@aid", appointmentId);
                        ...
                        using (SqlDataReader r = cmd.ExecuteReader())
                        {
                            if (r.Read())
                            {
                                patUserId = Convert.ToInt32(r["UserId"]);
                    ...
                    for (DateTime day = fromDate.Date; day <= toDate.Date; day = day.AddDays(1))
                    {
                        ...
                        while (cursor.AddHours(1) <= dayEnd)
                        {
                            ...
                            using (var cmd = new SqlCommand(@"
                                SELECT COUNT(1)
                                FROM [dbo].[DoctorAvailability]
                                WHERE DoctorId=@did AND StartTime=@s", conn))
                            {
                                cmd.Parameters.AddWithValue("@did", doctorId);
                                cmd.Parameters.AddWithValue("@s", cursor);
                ...
                try
                {
                    ...
                    using (var cmd = new SqlCommand(@"
                        ...
                        WHERE DoctorId=@did
                          ...
                          AND Status IN ('Pending','Accepted')", conn))
                    {
                        cmd.Parameters.AddWithValue("@did", doctorId);
                        cmd.Parameters.AddWithValue("@s", slotStart);
                    ...
                    if (hadBooking)
                    {
                        ...
                        using (var cmd = new SqlCommand(@"
                            ...
                            WHERE a.AppointmentId=@aid", conn))
                        {
                            cmd.Parameters.AddWithValue("@aid", bookedId);
        ...
        protected void rptSlots_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            using (var conn = new SqlConnection(_conn))
            {
                try
                {
                    ...
                    Execute(conn,
                        @"DELETE FROM [dbo].[DoctorAvailability]
                          WHERE AvailabilityId=@aid AND DoctorId=@did",
                        ("@aid", availabilityId),
                        ("@did", doctorId));
        ...
        private static void InsertNotification(SqlConnection conn,
            int userId,
            string type,
            string title,
            string message)
